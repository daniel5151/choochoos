...
void render_initial_screen(int uart, const TermSize& term_size) {
    ...
    int scroll_start = MAX_TRAINS + 3;
    int scroll_end = term_size.height - 1;
    int sep_row = scroll_start - 1;

    Uart::Printf(
        uart, COM2, VT_CLEAR VT_SET_SCROLL_FMT VT_ROWCOL_FMT "%s" VT_ROWCOL_FMT,
        scroll_start, scroll_end, sep_row, 1, sep, term_size.height - 1, 1);
}
