...
The next sensor and most recent error calcuations are also stored on the train
descriptor.
...
of continuous calls to `update_sensors` and `tick`, we can hopefully wake the
calling task at the right time.
